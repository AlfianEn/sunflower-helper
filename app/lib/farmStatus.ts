...
export type TimedItem = { type:string; id:string; name:string; readyAt:number; ready:boolean; detail?:string }
...
export function parseFarm(snapshot: FarmSnapshot | null): FarmStatus {
  ...
  const root=JSON.parse(snapshot.json); const farm=root.farm||root; const now=Date.now(); const inv=farm.inventory||{}
  const resources:TimedItem[]=[]
  const addTree=()=>{ for(const [id,node] of Object.entries<any>(farm.trees||{})){ const t=n(node?.wood?.choppedAt); if(t){ const readyAt=t+2*60*60*1000; resources.push({type:'resource',id:`tree:${id}`,name:'Tree',readyAt,ready:now-t>2*60*60*1000}) } }}
  const addRock=(name:string,nodes:any,baseSec:number)=>{ for(const [id,node] of Object.entries<any>(nodes||{})){ const t=n(node?.stone?.minedAt); if(t){ const readyAt=t+baseSec*1000; resources.push({type:'resource',id:`${name}:${id}`,name,readyAt,ready:now-t>=baseSec*1000}) } }}
  ...
  const mushrooms:TimedItem[]=Object.entries<any>(farm.mushrooms?.mushrooms||{}).map(([id,m])=>({type:'mushroom',id:`mushroom:${id}`,name:m.name||'Mushroom',readyAt:0,ready:true,detail:`x${m.amount||1}`}))
...
export function formatEta(ms:number){ if(ms<=Date.now()) return 'ready'; const m=Math.ceil((ms-Date.now())/60000); if(m<60) return `${m}m`; const h=Math.floor(m/60), r=m%60; return r?`${h}h ${r}m`:`${h}h` }
