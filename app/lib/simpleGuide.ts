...
import { parseFarm, formatEta, serverNow } from './farmStatus'
import { CROPS, profitPerHour } from './crops'
import { missingFor } from './crafting'

export type SimpleGuideStep = { title:string; where:string; why:string; done:string; tone?:'urgent'|'good'|'warn' }
export type SimpleGuide = { main:SimpleGuideStep; steps:SimpleGuideStep[]; status:ReturnType<typeof parseFarm> }
...
export function buildSimpleGuide(autoPlans:AutoCropPlan[], inventory:InventoryItem[], targets:CraftTarget[], snapshot:FarmSnapshot|null){
  const inv=Object.fromEntries(inventory.map(i=>[i.name,Number(i.qty)])); const status=parseFarm(snapshot); const now=serverNow(); const steps:SimpleGuideStep[]=[]
  ...
  return { main:steps[0], steps:steps.slice(0,5), status } as SimpleGuide
}
