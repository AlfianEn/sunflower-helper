...
export default async function Page({ searchParams }: { searchParams?: Promise<Record<string,string>> }) {
  ...
  return <main className="wrap">
    ...
    <div className="grid">
      <section className="card"><h2>Settings</h2><form action={saveSettings} className="grid"><input name="farmId" defaultValue={settings.farmId || process.env.SUNFLOWER_FARM_ID || '3132688624394422'} placeholder="Farm ID"/><input name="telegramChatId" defaultValue={settings.telegramChatId || process.env.TELEGRAM_CHAT_ID || 'telegram:840251768'} placeholder="Telegram chat id"/><button>Save</button></form></section>
      <section className="card"><h2>Add crop timer</h2><form action={addCrop} className="grid"><select name="crop">{CROPS.map(c=><option key={c.name}>{c.name}</option>)}</select><input name="plotCount" type="number" min="1" defaultValue="1"/><input name="notes" placeholder="Notes"/><button>Add Timer</button></form></section>
    </div>
