...
export default async function Page({ searchParams }: { searchParams?: Promise<Record<string,string>> }) {
  ...
  return <main className="wrap">
    ...
    <section className="card compact" style={{marginTop:14}}><h2>Detail: Cooking / Resources / Delivery</h2><div className="grid"><div className="pill">Cooking ready: {farmStatus.cooking.filter(c=>c.ready).length}</div><div className="pill">Resources ready: {farmStatus.resources.filter(r=>r.ready).length}</div><div className="pill">Delivery bisa kirim: {farmStatus.deliveries.filter(d=>!d.completed && d.fulfillable).length}</div><div className="pill">Open chores: {farmStatus.chores.length}</div></div></section>
