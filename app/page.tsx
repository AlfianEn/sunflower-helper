...
export default async function Page({ searchParams }: { searchParams?: Promise<Record<string,string>> }) {
  ...
  return <main className="wrap">
    ...
    <section className="card" style={{marginBottom:14}}><h2>Yang harus kamu lakukan sekarang</h2><div className="grid">{coach.map((a,i)=><div key={i} className={`action ${a.priority}`}><b>{a.title}</b><p>{a.detail}</p></div>)}</div><p className="muted" style={{marginTop:12}}>Ini guide aktif: baca farm kamu, cek crop ready, cek target craft, lalu kasih prioritas tindakan.</p></section>
    <div className="grid">
      <section className="card"><h2>Status sistem</h2><p className="muted">Auto-sync farm + inventory aktif. Guide berubah sesuai goal.</p><div className="row"><span className="pill ok">Auto sync: ON</span><span className="pill ok">Telegram: ON</span><span className="pill">Farm ID: {settings.farmId || process.env.SUNFLOWER_FARM_ID || '3132688624394422'}</span><span className="pill">Last sync: {snapshot ? new Date(snapshot.fetchedAt).toLocaleTimeString() : 'none'}</span></div><form action={setGoal} className="row" style={{marginTop:12}}><select name="goal" defaultValue={settings.goal || 'balanced'}><option value="balanced">Balanced</option><option value="profit">Profit</option><option value="craft">Craft target</option><option value="level">Level/XP</option></select><button>Set Goal</button></form><form action={testTelegram} style={{marginTop:12}}><button type="submit">Test Telegram</button></form></section>
      <section className="card"><h2>Manual timer fallback</h2><p className="muted">Opsional saja. Pakai ini kalau API game sedang telat/salah baca.</p><form action={addCrop} className="grid"><select name="crop">{CROPS.map(c=><option key={c.name}>{c.name}</option>)}</select><input name="plotCount" type="number" min="1" defaultValue="1"/><input name="plantedAt" type="datetime-local" title="Planted time, optional"/><input name="notes" placeholder="Notes"/><button>Add Manual Timer</button></form></section>
    </div>
