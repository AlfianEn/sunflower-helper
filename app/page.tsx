...
export default async function Page({ searchParams }: { searchParams?: Promise<Record<string,string>> }) {
  ...
  const coach = buildCoach(autoPlans, inventory, targets)
  const now = Date.now()
  return <main className="wrap">
    ...
    <div className="grid">
      <section className="card"><h2>Status sistem</h2><p className="muted">Telegram reminder sudah aktif otomatis ke chat ini. Kamu tidak perlu isi apa pun. Cukup tambah crop timer.</p><div className="row"><span className="pill ok">Ready reminder: ON</span><span className="pill ok">Before-ready: 5 menit</span><span className="pill">Farm ID: {settings.farmId || process.env.SUNFLOWER_FARM_ID || '3132688624394422'}</span></div><form action={testTelegram} style={{marginTop:12}}><button type="submit">Test Telegram</button></form></section>
